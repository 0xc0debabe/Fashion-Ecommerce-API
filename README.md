...
- **상품 목록 조회** (메인 페이지)
    ...
    - 조회수 TOP 15는 상세 페이지에 접근 시, 조회수 랭킹에 +1점을 부여하여 책정(쿠키를 이용해 재방문 중복 제거)
...
### 📌 Order(조회 시, 페이징 처리되어서 응답)
- **주문생성 기능**
    - '멤버'만이 주문 가능
    - 개별 주문 : CreateOrderDto의 값이 null이 아닐 경우 개별 주문이라 판단하여 하나의 상품만 주문 됨
    - 장바구니 주문 : CreateOrderDto의 값이 null일 경우 장바구니 주문으로 판단하여
    - 주문 시, 주문 상태로 'PENDING' 상태로 되어있음
    - 해당 아이템 재고 -1
      
- **주문 목록 조회**
    - '멤버'만이 조회 가능
    - 멤버가 주문한 목록을 최신순으로 조회 가능
      
- **주문 취소 기능**
    - 'PENDING'처리 되어 있는 주문만을 취소할 수 있음
    - 주문 취소시 해당 아이템 재고 +1
      
- **판매자의 주문 목록 조회**
    - '셀러'만이 조회가능
    - 주문 상태로 'PENDING', 'CANCELED', 'COMPLETED'가 있는데 동적쿼리를 이용해 해당 주문 상태 조회 가능
    - Ex) PENDING, CANCELED 선택 시, PENDING이나 CANCELED의 주문 상태 확인 가능
    - Ex) COMPLETED 선택 시, COMPLETED의 주문 상태만 확인 가능
      
- **판매자 주문 완료**
    - 판매자만 자신의 판매 주문을 완료할 수 있음

### 📌 Review(조회 시, 페이징 처리되어서 응답, 리뷰를 끌어올 때 멤버의 정보도 끌어오기 때문에 QueryDSL을 사용하여 n+1 문제를 해결하고 이를 통해 성능 이슈를 최소화)
- **리뷰 생성**
    - 인증된 회원만 상품에 리뷰를 생성
      
- **리뷰 수정**
    - 댓글이 본인이 쓴 게 맞는 지 확인 후, 작성된 리뷰를 수정
      
- **리뷰 삭제**
    - 댓글이 본인이 쓴 게 맞는 지 확인 후, 작성된 리뷰를 삭제
    
- **리뷰 조회 (최신순)**
    - 상품에 대한 최신 리뷰 목록을 조회
      
- **리뷰 조회 (오래된순)**
    - 상품에 대한 오래된 리뷰 목록을 조회
    
- **리뷰 조회 (평점 오름차순)**
    - 상품에 대한 평점 오름차순 리뷰 목록을 조회
    
- **리뷰 조회 (평점 내림차순)**
    - 상품에 대한 평점 내림차순 리뷰 목록을 조회
